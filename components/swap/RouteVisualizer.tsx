...
const PALETTE = ["#2F6FED", "#5A8DF2", "#8FB2F7", "#0D47A1", "#C2D6FB"];
...
export function RouteVisualizer({ fills, loading }: { fills?: Fill[]; loading?: boolean }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 rounded-xl2 border border-surface-700 bg-surface-800/40 px-3.5 py-3">
        <div className="h-2 flex-1 animate-pulse rounded-full bg-surface-600" />
        <span className="text-xs text-ivory-faint">Finding best route…</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl2 border border-surface-700 bg-surface-800/40 px-3.5 py-3">
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-x-3 gap-y-1">
        {segments.slice(0, 4).map((s, i) => (
          <span key={s.source + i} className="flex items-center gap-1.5 text-[11px] text-ivory-muted">
            <span className="h-1.5 w-1.5 rounded-full" style={{ background: s.color }} />
            {s.source.replace(/_/g, " ")} · {s.pct.toFixed(0)}%
          </span>
        ))}
        {segments.length > 4 && <span className="text-[11px] text-ivory-faint">+{segments.length - 4} more</span>}
      </div>
    </div>
  );
}
