        ...
        >
          ...
          >
            ...
            {!isTerminal && needsApproval && (
              <div className="mt-5 flex items-center justify-center gap-2 text-xs text-ivory-faint">
            ...
            {txHash && chainId && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-4 inline-flex items-center gap-1.5 text-xs font-medium text-core-300 hover:text-core-200"
              >
                View {truncateAddress(txHash)} on explorer
                <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
                  <path d="M3 9l6-6M9 3H4M9 3v5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </a>
            )}
            ...
            {isTerminal && (
              <div className="mt-6 flex gap-2">
                ...
                <button
                  onClick={onClose}
                  className="flex-1 rounded-full border border-surface-600 py-2.5 text-sm font-medium text-ivory-muted transition-colors hover:text-ivory"
                >
  ...
  return (
    <span
      className={`h-1.5 w-1.5 rounded-full transition-colors ${
        done ? "bg-success-500" : active ? "bg-core-400 animate-pulse" : "bg-surface-600"
      }`}
    />
  );
}
